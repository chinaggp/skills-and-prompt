...
description: Route general software development work by phase so analysis tasks use GPT-5.4 and implementation tasks use GPT-5.6-terra with medium reasoning effort. Use when handling requirement analysis, solution design, code changes, bug fixes, refactors, test updates, build fixes, or mixed requests that move from understanding into implementation.
---
...
Choose `gpt-5.6-terra` with `reasoning_effort: "medium"` for coding phase work:
...
2. Switch to `gpt-5.6-terra` with medium reasoning effort once the implementation target is concrete.
...
- `Current phase: implementation, model: gpt-5.6-terra, reasoning_effort: medium`

Do not claim that a model switch happened unless the implementation agent was created with explicit `model` and `reasoning_effort` fields.
...
Use `gpt-5.6-terra` with medium reasoning effort once direction is settled:
...
For mixed requests such as "analyze first, then implement directly", begin with `gpt-5.4`, summarize the chosen implementation direction, then continue with `gpt-5.6-terra` using medium reasoning effort.
...
3. Create an implementation subagent with `spawn_agent(... model="gpt-5.6-terra", reasoning_effort="medium" ...)`
4. Announce `Current phase: implementation, model: gpt-5.6-terra, reasoning_effort: medium`
